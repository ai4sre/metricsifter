...
class TestRun:
    def test_writes_output_csv(self, tmp_path, input_csv):
        ...
        code = cli.main(["run", str(input_csv), "--output", str(out), "--n-jobs", "1"])
    ...
    def test_writes_report_json(self, tmp_path, input_csv):
        ...
        code = cli.main(["run", str(input_csv), "--output", str(out), "--report", str(report), "--n-jobs", "1"])
    ...
    def test_stdout_when_no_output(self, capsys, input_csv):
        code = cli.main(["run", str(input_csv), "--n-jobs", "1"])
        assert code == cli.EXIT_OK
